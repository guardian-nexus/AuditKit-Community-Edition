...
// Ensure Lambda functions are in VPC when accessing VPC resources
func (c *LambdaChecks) CheckLambdaInVPC(ctx context.Context) (CheckResult, error) {
  ...
  --vpc-config SubnetIds=subnet-xxx,SecurityGroupIds=sg-xxx`,
	...
	return CheckResult{
		...
		Name:       "Lambda Functions in VPC",
		Status:     "PASS",
		Evidence:   fmt.Sprintf("All %d Lambda functions are in VPC", totalFunctions),
...
func (c *LambdaChecks) CheckLambdaEnvironmentEncryption(ctx context.Context) (CheckResult, error) {
	...
	if len(functionsWithoutKMS) > 0 {
		...
		return CheckResult{
			...
			Status:      "FAIL",
			Severity:    "HIGH",
			Evidence:    fmt.Sprintf("%d/%d functions with environment variables not encrypted with customer KMS key: %v", len(functionsWithoutKMS), totalWithEnvVars, displayFunctions),
  ...
  --kms-key-arn arn:aws:kms:region:account:key/KEY_ID`,
	...
	if totalWithEnvVars == 0 {
		return CheckResult{
			...
			Name:       "Lambda Environment Encryption",
			Status:     "PASS",
			Evidence:   "No Lambda functions with environment variables",
			Priority:   PriorityInfo,
			Timestamp:  time.Now(),
	...
	return CheckResult{
		...
		Name:       "Lambda Environment Encryption",
		Status:     "PASS",
		Evidence:   fmt.Sprintf("All %d functions with environment variables use KMS encryption", totalWithEnvVars),
		Priority:   PriorityInfo,
		Timestamp:  time.Now(),
		Frameworks: map[string]string{"SOC2": "CC6.7"},
	}, nil
...
func (c *LambdaChecks) CheckLambdaExecutionRole(ctx context.Context) (CheckResult, error) {
	...
	if len(functionsWithBroadRoles) > 0 {
		...
		return CheckResult{
			...
			Status:      "FAIL",
			Severity:    "HIGH",
			Evidence:    fmt.Sprintf("%d functions with overly permissive roles: %v", len(functionsWithBroadRoles), displayFunctions),
...
aws lambda update-function-configuration \
  ...
  --role arn:aws:iam::ACCOUNT:role/LambdaExecutionRole`,
	...
	return CheckResult{
		...
		Name:       "Lambda Execution Role Permissions",
		Status:     "PASS",
		Evidence:   "Lambda functions use least-privilege execution roles",
		Priority:   PriorityInfo,
		Timestamp:  time.Now(),
...
func (c *LambdaChecks) CheckLambdaPublicAccess(ctx context.Context) (CheckResult, error) {
	...
	if len(publicFunctions) > 0 {
		...
		return CheckResult{
			...
			Status:      "FAIL",
			Severity:    "CRITICAL",
			Evidence:    fmt.Sprintf("%d Lambda functions are publicly accessible: %v", len(publicFunctions), displayFunctions),
  ...
  --statement-id AllowPublicInvoke`,
	...
	return CheckResult{
		...
		Name:       "Lambda Functions Not Public",
		Status:     "PASS",
		Evidence:   "No Lambda functions are publicly accessible",
		Priority:   PriorityInfo,
		Timestamp:  time.Now(),
...
func (c *LambdaChecks) CheckLambdaTracing(ctx context.Context) (CheckResult, error) {
	...
	if len(functionsWithoutTracing) > 0 {
		...
		return CheckResult{
			...
			Status:      "FAIL",
			Severity:    "LOW",
			Evidence:    fmt.Sprintf("%d/%d functions without X-Ray tracing: %v", len(functionsWithoutTracing), totalFunctions, displayFunctions),
  ...
  --tracing-config Mode=Active`,
	...
	return CheckResult{
		...
		Name:       "Lambda X-Ray Tracing Enabled",
		Status:     "PASS",
		Evidence:   fmt.Sprintf("All %d Lambda functions have X-Ray tracing enabled", totalFunctions),
		Priority:   PriorityInfo,
		Timestamp:  time.Now(),
		Frameworks: map[string]string{"SOC2": "CC7.2"},
	}, nil
