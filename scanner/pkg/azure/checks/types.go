package checks

import (
	"context"
	"fmt"
	"time"
)

// Check result statuses. Only StatusPass and StatusFail are scoreable; every
// other value is excluded from the compliance score rather than counted as a
// failed automated check.
//
// Three separate defects came from this field being an unconstrained string:
// MANUAL, then WARN, then ERROR each sat in the scoring denominator and could
// never pass. .github/scripts/check-statuses.sh rejects any value outside this
// set, so a fourth cannot ship.
const (
	StatusPass   = "PASS"   // the control was checked and satisfied
	StatusFail   = "FAIL"   // the control was checked and not satisfied
	StatusInfo   = "INFO"   // guidance only; nothing was evaluated
	StatusManual = "MANUAL" // requires human verification or documentation
	StatusError  = "ERROR"  // the check could not run, usually a missing permission
)

// ValidStatus reports whether s is one of the recognised statuses.
func ValidStatus(s string) bool {
	switch s {
	case StatusPass, StatusFail, StatusInfo, StatusManual, StatusError:
		return true
	}
	return false
}

// Framework constants - same as AWS
const (
	FrameworkSOC2     = "SOC2"
	FrameworkPCI      = "PCI-DSS"
	FrameworkHIPAA    = "HIPAA"
	FrameworkISO      = "ISO27001" // must match the crosswalk key exactly
	FrameworkCISAzure = "CIS-Azure"
)

// CheckResult - identical structure to AWS
type CheckResult struct {
	Control           string            `json:"control"`
	Name              string            `json:"name"`
	Status            string            `json:"status"` // PASS, FAIL, NOT_APPLICABLE, ERROR
	Evidence          string            `json:"evidence"`
	Remediation       string            `json:"remediation,omitempty"`
	RemediationDetail string            `json:"remediation_detail,omitempty"`
	Severity          string            `json:"severity,omitempty"`
	Priority          Priority          `json:"priority"`
	ScreenshotGuide   string            `json:"screenshot_guide,omitempty"`
	ConsoleURL        string            `json:"console_url,omitempty"`
	Timestamp         time.Time         `json:"timestamp"`
	Frameworks        map[string]string `json:"frameworks,omitempty"`
}

// Priority - NO EMOJIS
type Priority struct {
	Level     string `json:"level"`
	Impact    string `json:"impact"`
	TimeToFix string `json:"time_to_fix"`
	WillFail  bool   `json:"will_fail_audit"`
}

// Check interface - same as AWS
type Check interface {
	Run(ctx context.Context) ([]CheckResult, error)
	Name() string
}

// Azure-specific framework mappings. The CIS edition is recorded in
// pkg/mappings, not asserted here.
var AzureFrameworkMappings = map[string]map[string]string{
	// Identity and Access Management (Section 1)
	"AAD_MFA": {
		FrameworkSOC2:     "CC6.6",
		FrameworkPCI:      "8.4.2",
		FrameworkHIPAA:    "164.312(a)(2)(i)",
		FrameworkISO:      "A.8.5",
		FrameworkCISAzure: "5.1.3",
	},
	"AAD_PASSWORD_POLICY": {
		FrameworkSOC2:  "CC6.7",
		FrameworkPCI:   "8.3.6, 8.3.9, 8.3.7",
		FrameworkHIPAA: "164.308(a)(5)(ii)(D)",
		FrameworkISO:   "A.5.17",
	},
	"AAD_PRIVILEGED_ROLES": {
		FrameworkSOC2:     "CC6.2",
		FrameworkPCI:      "7.2.2",
		FrameworkHIPAA:    "164.308(a)(3)(ii)(A)",
		FrameworkISO:      "A.8.2",
		FrameworkCISAzure: "5.7",
	},
	"AAD_GUEST_USERS": {
		FrameworkSOC2:  "CC6.1",
		FrameworkPCI:   "7.1.1",
		FrameworkHIPAA: "164.308(a)(4)(ii)(C)",
		FrameworkISO:   "A.5.18",
		// aad.go carries Control: "CIS-5.3.2" on the check that lists the
		// guests, so claiming it here too was a duplicate - and Pro has no
		// AAD_GUEST_USERS entry, which made the parity guard read the duplicate
		// as coverage Pro lacked.
	},
	"AAD_CONDITIONAL_ACCESS": {
		FrameworkSOC2:  "CC6.6",
		FrameworkPCI:   "8.3.2",
		FrameworkHIPAA: "164.312(a)(2)(i)",
		FrameworkISO:   "A.8.5",
	},

	// Microsoft Defender for Cloud (Section 2) - ENHANCED with granular mappings
	"DEFENDER_SERVERS": {
		FrameworkSOC2:     "CC7.2",
		FrameworkPCI:      "11.5.2",
		FrameworkHIPAA:    "164.308(a)(1)(ii)(A)",
		FrameworkISO:      "A.8.8",
		FrameworkCISAzure: "8.1.3.1",
	},
	"DEFENDER_APPSERVICE": {
		FrameworkSOC2:     "CC7.2",
		FrameworkPCI:      "11.5.2",
		FrameworkHIPAA:    "164.308(a)(1)(ii)(A)",
		FrameworkISO:      "A.8.8",
		FrameworkCISAzure: "8.1.6.1",
	},
	"DEFENDER_SQL": {
		FrameworkSOC2:     "CC7.2",
		FrameworkPCI:      "11.5.2",
		FrameworkHIPAA:    "164.308(a)(1)(ii)(A)",
		FrameworkISO:      "A.8.8",
		FrameworkCISAzure: "8.1.7.3",
	},
	"DEFENDER_SQL_VM": {
		FrameworkSOC2:     "CC7.2",
		FrameworkPCI:      "11.5.2",
		FrameworkHIPAA:    "164.308(a)(1)(ii)(A)",
		FrameworkISO:      "A.8.8",
		FrameworkCISAzure: "8.1.7.4",
	},
	"DEFENDER_OPENSOURCE_DB": {
		FrameworkSOC2:     "CC7.2",
		FrameworkPCI:      "11.5.2",
		FrameworkHIPAA:    "164.308(a)(1)(ii)(A)",
		FrameworkISO:      "A.8.8",
		FrameworkCISAzure: "8.1.7.2",
	},
	"DEFENDER_COSMOSDB": {
		FrameworkSOC2:     "CC7.2",
		FrameworkPCI:      "11.5.2",
		FrameworkHIPAA:    "164.308(a)(1)(ii)(A)",
		FrameworkISO:      "A.8.8",
		FrameworkCISAzure: "8.1.7.1",
	},
	"DEFENDER_STORAGE": {
		FrameworkSOC2:     "CC7.2",
		FrameworkPCI:      "11.5.2",
		FrameworkHIPAA:    "164.308(a)(1)(ii)(A)",
		FrameworkISO:      "A.8.8",
		FrameworkCISAzure: "8.1.5.1",
	},
	"DEFENDER_CONTAINERS": {
		FrameworkSOC2:     "CC7.2",
		FrameworkPCI:      "11.5.2",
		FrameworkHIPAA:    "164.308(a)(1)(ii)(A)",
		FrameworkISO:      "A.8.8",
		FrameworkCISAzure: "8.1.4.1",
	},
	"DEFENDER_DNS": {
		FrameworkSOC2:  "CC7.2",
		FrameworkPCI:   "11.5.1",
		FrameworkHIPAA: "164.308(a)(1)(ii)(A)",
		FrameworkISO:   "A.8.8",
	},
	"DEFENDER_KEYVAULT": {
		FrameworkSOC2:     "CC6.6",
		FrameworkPCI:      "11.5.2",
		FrameworkHIPAA:    "164.308(a)(1)(ii)(A)",
		FrameworkISO:      "A.8.8",
		FrameworkCISAzure: "8.1.8.1",
	},
	"DEFENDER_API": {
		FrameworkSOC2:     "CC7.2",
		FrameworkPCI:      "6.2.4",
		FrameworkHIPAA:    "164.308(a)(1)(ii)(A)",
		FrameworkISO:      "A.8.8",
		FrameworkCISAzure: "8.1.2.1",
	},
	"DEFENDER_ARM": {
		FrameworkSOC2:     "CC7.2",
		FrameworkPCI:      "11.5.2",
		FrameworkHIPAA:    "164.308(a)(1)(ii)(A)",
		FrameworkISO:      "A.8.8",
		FrameworkCISAzure: "8.1.9.1",
	},
	"DEFENDER_AUTOPROVISION": { // Alias
		FrameworkSOC2:  "CC7.3",
		FrameworkPCI:   "11.5.2",
		FrameworkHIPAA: "164.308(a)(1)(ii)(A)",
		FrameworkISO:   "A.8.8",
	},
	"DEFENDER_CONTACTS": { // Alias
		FrameworkSOC2:     "CC7.4",
		FrameworkPCI:      "12.10.1",
		FrameworkHIPAA:    "164.308(a)(6)(ii)",
		FrameworkISO:      "A.6.8",
		FrameworkCISAzure: "8.1.13",
	},
	"DEFENDER_ALERTS": {
		FrameworkSOC2:     "CC7.4",
		FrameworkPCI:      "12.10.1",
		FrameworkHIPAA:    "164.308(a)(6)(ii)",
		FrameworkISO:      "A.6.8",
		FrameworkCISAzure: "8.1.14",
	},

	// Storage Accounts (Section 3) - ENHANCED with new checks
	"STORAGE_PUBLIC_ACCESS": {
		FrameworkSOC2:     "CC6.2",
		FrameworkPCI:      "1.4.2, 1.3.2",
		FrameworkHIPAA:    "164.312(a)(1)",
		FrameworkISO:      "A.8.20",
		FrameworkCISAzure: "9.3.2.2, 9.3.8",
	},
	"STORAGE_ENCRYPTION": {
		FrameworkSOC2:  "CC6.3",
		FrameworkPCI:   "3.5.1",
		FrameworkHIPAA: "164.312(a)(2)(iv)",
		FrameworkISO:   "A.8.24",
	},
	"STORAGE_SECURE_TRANSFER": {
		FrameworkSOC2:     "CC6.7",
		FrameworkPCI:      "4.2.1",
		FrameworkHIPAA:    "164.312(e)(1)",
		FrameworkISO:      "A.8.20",
		FrameworkCISAzure: "9.3.4",
	},
	"STORAGE_SOFT_DELETE": {
		FrameworkSOC2:     "CC9.1",
		FrameworkPCI:      "3.2.1",
		FrameworkHIPAA:    "164.308(a)(7)(ii)(A)",
		FrameworkISO:      "A.8.13",
		FrameworkCISAzure: "9.2.1, 9.2.2",
	},
	"STORAGE_NETWORK_RULES": {
		FrameworkSOC2:     "CC6.1",
		FrameworkPCI:      "1.4.2",
		FrameworkHIPAA:    "164.312(e)(1)",
		FrameworkISO:      "A.8.22",
		FrameworkCISAzure: "9.3.2.2, 9.3.2.3",
	},

	// Database Services (Section 4/5) - ENHANCED
	"SQL_TDE": {
		FrameworkSOC2:  "CC6.3",
		FrameworkPCI:   "3.5.1",
		FrameworkHIPAA: "164.312(a)(2)(iv)",
		FrameworkISO:   "A.8.24",
	},
	"SQL_AUDITING": {
		FrameworkSOC2:  "CC7.1",
		FrameworkPCI:   "10.2.1.1",
		FrameworkHIPAA: "164.312(b)",
		FrameworkISO:   "A.8.15",
	},
	"SQL_THREAT_DETECTION": {
		FrameworkSOC2:  "CC7.2",
		FrameworkPCI:   "11.5.1",
		FrameworkHIPAA: "164.308(a)(1)(ii)(A)",
		FrameworkISO:   "A.8.8",
	},
	"SQL_FIREWALL": {
		FrameworkSOC2:  "CC6.1",
		FrameworkPCI:   "1.4.2",
		FrameworkHIPAA: "164.312(e)(1)",
		FrameworkISO:   "A.8.22",
	},
	"SQL_ENTRA_AUTH": {
		FrameworkSOC2:  "CC6.1",
		FrameworkPCI:   "8.2.2",
		FrameworkHIPAA: "164.312(a)(2)(i)",
		FrameworkISO:   "A.8.5",
	},

	// Logging and Monitoring (Section 5)
	"ACTIVITY_LOG": {
		FrameworkSOC2:     "CC7.1",
		FrameworkPCI:      "10.2.1, 10.2.1.1",
		FrameworkHIPAA:    "164.312(b)",
		FrameworkISO:      "A.8.15",
		FrameworkCISAzure: "6.1.1.1, 6.1.1.2",
	},
	"DIAGNOSTIC_SETTINGS": {
		FrameworkSOC2:     "CC7.2",
		FrameworkPCI:      "10.2.1.1",
		FrameworkHIPAA:    "164.312(b)",
		FrameworkISO:      "A.8.15",
		FrameworkCISAzure: "6.1.1.1, 6.1.1.2",
	},

	// Networking (Section 6/7) - ENHANCED with specific ports
	"NSG_RULES": {
		FrameworkSOC2:     "CC6.1",
		FrameworkPCI:      "1.4.2, 1.4.1",
		FrameworkHIPAA:    "164.312(e)(1)",
		FrameworkISO:      "A.8.22",
		FrameworkCISAzure: "7.1, 7.2, 7.3",
	},
	"NSG_FLOW_LOGS": {
		FrameworkSOC2:     "CC7.2",
		FrameworkPCI:      "10.2.1.1",
		FrameworkHIPAA:    "164.312(b)",
		FrameworkISO:      "A.8.20",
		FrameworkCISAzure: "7.5",
	},
	"NETWORK_WATCHER": {
		FrameworkSOC2:     "CC7.2",
		FrameworkPCI:      "11.5.1",
		FrameworkHIPAA:    "164.312(b)",
		FrameworkISO:      "A.8.20",
		FrameworkCISAzure: "7.6",
	},

	// Virtual Machines (Section 7) - ENHANCED
	"DISK_ENCRYPTION": {
		FrameworkSOC2:  "CC6.3",
		FrameworkPCI:   "3.5.1",
		FrameworkHIPAA: "164.312(a)(2)(iv)",
		FrameworkISO:   "A.8.24",
	},
	"VM_MANAGED_DISKS": {
		FrameworkSOC2:  "CC6.3",
		FrameworkPCI:   "3.5.1",
		FrameworkHIPAA: "164.312(a)(2)(iv)",
		FrameworkISO:   "A.8.24",
	},
	"VM_ENDPOINT_PROTECTION": {
		FrameworkSOC2:     "CC7.2",
		FrameworkPCI:      "5.2.1",
		FrameworkHIPAA:    "164.308(a)(5)(ii)(B)",
		FrameworkISO:      "A.8.7",
		FrameworkCISAzure: "8.1.3.3",
	},
	"VM_BACKUP": {
		FrameworkSOC2:  "CC9.1",
		FrameworkPCI:   "9.1.1",
		FrameworkHIPAA: "164.308(a)(7)(ii)(A)",
		FrameworkISO:   "A.8.13",
	},
	"DISK_NETWORK_ACCESS": {
		FrameworkSOC2:  "CC6.1",
		FrameworkPCI:   "1.4.2, 1.3.2",
		FrameworkHIPAA: "164.312(a)(1)",
		FrameworkISO:   "A.8.20",
	},

	// Key Vault (Section 8) - ENHANCED
	"KEYVAULT_PURGE": {
		FrameworkSOC2:     "CC6.3",
		FrameworkPCI:      "3.5.1",
		FrameworkHIPAA:    "164.312(a)(2)(iv)",
		FrameworkISO:      "A.8.24",
		FrameworkCISAzure: "8.3.5",
	},
	"KEYVAULT_RBAC": {
		FrameworkSOC2:     "CC6.2",
		FrameworkPCI:      "7.2.2",
		FrameworkHIPAA:    "164.308(a)(3)(ii)(A)",
		FrameworkISO:      "A.8.2",
		FrameworkCISAzure: "8.3.6",
	},
	"KEYVAULT_PRIVATE_ENDPOINT": {
		FrameworkSOC2:     "CC6.1",
		FrameworkPCI:      "1.4.2",
		FrameworkHIPAA:    "164.312(e)(1)",
		FrameworkISO:      "A.8.22",
		FrameworkCISAzure: "8.3.8",
	},
	"KEYVAULT_NETWORK_RULES": { // Alias
		FrameworkSOC2:     "CC6.1",
		FrameworkPCI:      "1.4.2",
		FrameworkHIPAA:    "164.312(e)(1)",
		FrameworkISO:      "A.8.22",
		FrameworkCISAzure: "8.3.7",
	},
	"KEYVAULT_LOGGING": {
		FrameworkSOC2:     "CC7.1",
		FrameworkPCI:      "10.2.1.1",
		FrameworkHIPAA:    "164.312(b)",
		FrameworkISO:      "A.8.15",
		FrameworkCISAzure: "6.1.1.4",
	},
}

// Helper function to get framework mappings for a control
func GetFrameworkMappings(controlType string) map[string]string {
	if mappings, exists := AzureFrameworkMappings[controlType]; exists {
		return mappings
	}
	return make(map[string]string)
}

// Helper to format framework requirements in evidence
func FormatFrameworkRequirements(frameworks map[string]string) string {
	if len(frameworks) == 0 {
		return ""
	}

	result := " | Requirements: "
	for fw, requirement := range frameworks {
		result += fmt.Sprintf("%s %s, ", fw, requirement)
	}
	// Remove trailing comma and space
	if len(result) > 2 {
		result = result[:len(result)-2]
	}
	return result
}

// Priority definitions - EXACTLY matching AWS, NO EMOJIS
var (
	PriorityCritical = Priority{
		Level:     "CRITICAL",
		Impact:    "AUDIT BLOCKER - Fix immediately or fail audit",
		TimeToFix: "Fix RIGHT NOW",
		WillFail:  true,
	}

	PriorityHigh = Priority{
		Level:     "HIGH",
		Impact:    "Major finding - Auditor will flag this",
		TimeToFix: "Fix this week",
		WillFail:  false,
	}

	PriorityMedium = Priority{
		Level:     "MEDIUM",
		Impact:    "Should fix - Makes audit smoother",
		TimeToFix: "Fix before audit",
		WillFail:  false,
	}

	PriorityLow = Priority{
		Level:     "LOW",
		Impact:    "Nice to have - Strengthens posture",
		TimeToFix: "When convenient",
		WillFail:  false,
	}

	PriorityInfo = Priority{
		Level:     "INFO",
		Impact:    "Good job, this passes",
		TimeToFix: "Already done",
		WillFail:  false,
	}
)

// Common evidence message prefixes without emojis
const (
	CriticalViolation = "CRITICAL:"
	HighRisk          = "HIGH RISK:"
	MediumRisk        = "MEDIUM RISK:"
	Compliant         = "COMPLIANT:"
	ManualReview      = "MANUAL REVIEW REQUIRED:"
	CheckError        = "ERROR:"
)

func min(a, b int) int {
	if a < b {
		return a
	}
	return b
}
