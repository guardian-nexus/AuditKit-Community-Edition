...
ScubaGear names its consolidated output `ScubaResults_<uuid>.json`, so the glob
below picks it up. If a directory holds more than one run, name the file you
want explicitly.
...
- `Results` is keyed by ScubaGear's baseline names - `AAD`, `EXO`, `SharePoint`,
  `Teams`, `PowerPlatform`, `PowerBI` - not by the lower-case product arguments
  you pass to `-ProductNames`.
- The policy identifier is in **`Control ID`**; `Requirement` holds the prose.
- `Result` is a string: `Pass`, `Fail`, `Warning`, `Error`, `Omitted` or
  `Incorrect result`. AuditKit scores `Pass` and `Fail`; `Omitted` and `Error`
  are reported but excluded from the score, so a policy your team chose not to
  evaluate does not count against you.
- Policy ids carry a baseline version suffix (`MS.AAD.1.1v1`). AuditKit matches
  on the identifier with or without it.
...
Pass the consolidated `ScubaResults_<uuid>.json`, not a per-product report from
`IndividualReports/` and not `BaselineReports.html`.
...
AuditKit only imports policies it has a mapping for - 134 rules across Entra,
Exchange Online, SharePoint, Teams, Power Platform, Power BI and Defender.
A product you scanned that AuditKit has no mappings for is skipped silently.
Check that the products you ran are among those listed under
[Covered Domains](../providers/m365.md).
